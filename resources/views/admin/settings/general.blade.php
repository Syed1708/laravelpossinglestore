            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 12px; margin-bottom: 20px;">
                    <div class="toggle-row">
                        <div>
                            ...
                            <span style="font-size: 11px; color: var(--muted-foreground);">Uncheck to force store closed</span>
                        </div>
                        <input type="checkbox" name="is_store_open" value="1" {{ $settings->is_store_open ? 'checked' : '' }} style="width: 20px; height: 20px; cursor: pointer;">
                    </div>

                    <div class="toggle-row">
                        <div>
                            <strong style="display: block; font-size: 13px;">Online Click &amp; Collect</strong>
                            <span style="font-size: 11px; color: var(--muted-foreground);">Allow web customer orders</span>
                        </div>
                        <input type="checkbox" name="online_orders_enabled" value="1" {{ $settings->online_orders_enabled ? 'checked' : '' }} style="width: 20px; height: 20px; cursor: pointer;">
                    </div>

                    <div class="toggle-row">
                        <div>
                            <strong style="display: block; font-size: 13px;">Table Reservations</strong>
                            <span style="font-size: 11px; color: var(--muted-foreground);">Allow web table bookings</span>
                        </div>
                        <input type="checkbox" name="reservations_enabled" value="1" {{ $settings->reservations_enabled ? 'checked' : '' }} style="width: 20px; height: 20px; cursor: pointer;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 16px;">
                    <div class="form-group">
                        ...
                        <input type="time" name="shift1_start" class="form-input" value="{{ \Carbon\Carbon::parse($settings->shift1_start)->format('H:i') }}" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label" style="font-weight: bold;">Shift 1 End (HH:MM)</label>
                        <input type="time" name="shift1_end" class="form-input" value="{{ \Carbon\Carbon::parse($settings->shift1_end)->format('H:i') }}" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label" style="font-weight: bold;">Shift 2 Start (HH:MM)</label>
                        <input type="time" name="shift2_start" class="form-input" value="{{ \Carbon\Carbon::parse($settings->shift2_start)->format('H:i') }}" required>
                    </div>

                    <div class="form-group">
                        <label class="form-label" style="font-weight: bold;">Shift 2 End (HH:MM)</label>
                        <input type="time" name="shift2_end" class="form-input" value="{{ \Carbon\Carbon::parse($settings->shift2_end)->format('H:i') }}" required>
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label" style="font-weight: bold;">Closure Message Shown to Customers</label>
                    <input type="text" name="closed_message" class="form-input" value="{{ $settings->closed_message }}">
                </div>
            </div>
        </div>
