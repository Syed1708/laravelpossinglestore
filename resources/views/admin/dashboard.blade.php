    ...
    <div class="card" style="margin-bottom: 2rem;">
        <div class="card-body" style="padding: 1.25rem;">
            <form action="{{ route('tyro-dashboard.index') }}" method="GET">
                <div
                    style="display: flex; flex-direction: row; gap: 15px; align-items: flex-end; justify-content: flex-start; flex-wrap: wrap; width: 100%;">

                    <!-- Period Dropdown Container (No form-group class to prevent margin conflicts) -->
                    <div
                        style="flex: 1 1 180px; min-width: 150px; display: flex; flex-direction: column; margin-bottom: 0;">
                        <label
                            style="font-size: 0.875rem; font-weight: bold; margin-bottom: 6px; display: block; color: var(--foreground, #1a202c);">Période</label>
                        <select name="filter_type" id="filter_type" class="form-control" onchange="toggleDateInputs()"
                            style="height: 38px; border: 1px solid var(--border, #cbd5e0); border-radius: 6px; padding: 0.5rem 0.75rem; background-color: var(--background, #fff); color: var(--foreground, #1a202c);">
                            <option value="today" @selected($filterType === 'today')>Aujourd'hui</option>
                            <option value="week" @selected($filterType === 'week')>Cette Semaine (Hebdomadaire)</option>
                            <!-- 🚀 NEW -->
                    ...
                    <div id="custom-start-box"
                        style="display: {{ $filterType === 'custom' ? 'flex' : 'none' }}; flex: 1 1 180px; min-width: 150px; flex-direction: column; margin-bottom: 0;">
                        <label
                            style="font-size: 0.875rem; font-weight: bold; margin-bottom: 6px; display: block; color: var(--foreground, #1a202c);">Date
                            de Début</label>
                    ...
                    <div style="flex: 0 0 auto; margin-bottom: 0; display: flex; align-items: flex-end; height: 38px;">
                        <button type="submit" class="btn btn-primary"
                            style="height: 38px; padding: 0 1.5rem; font-weight: bold; display: flex; align-items: center; justify-content: center; margin: 0;">
                            🔍 Filtrer
                        </button>
                    </div>
                            ...
                            @foreach ($recentOrders as $order)
                                <tr>
                                    <td style="font-weight: bold;">
                                        ...
                                        @else
                                            <span style="color: var(--foreground);">Ticket
                                                #{{ $order->sequence_number }}</span>
                                        @endif
                                    </td>
                                    <td>{{ $order->completed_at->format('d/m/Y H:i') }}</td>
                                    <td
                                        style="font-weight: bold; {{ $order->total_incl_vat < 0 ? 'color: var(--destructive);' : 'color: var(--foreground);' }}">
