...
<head>
    ...
    <style>
        ...
        /* LEFT: HORIZONTAL SCROLL TICKETS */
        .kds-grid {
            display: flex;
            gap: 16px;
            overflow-x: auto;
        ...
        .status-pending { background-color: rgba(71, 85, 105, 0.4); color: #e2e8f0; }
        .status-preparing { background-color: rgba(245, 158, 11, 0.2); color: var(--warning); border-color: rgba(245, 158, 11, 0.4); }
        .status-ready { background-color: rgba(16, 185, 129, 0.25); color: #34d399; font-weight: 900; }
        .status-no-kitchen { background-color: rgba(59, 130, 246, 0.2); color: #93c5fd; }
        ...
        .kds-btn-complete { background-color: var(--success); color: #0b0f19; }
        ...
        /* 🎒 LIVE ITEMS TO PACK ACCUMULATOR */
...
<body>
    ...
    <!-- 1. HEADER WITH TABS, SEARCH, AND ACTION BUTTONS -->
        ...
        <div class="kds-title-group">
            ...
            <div class="filter-tabs">
                ...
                <button class="filter-btn" data-filter="ready" onclick="setPackerFilter('ready')">
                    ✔️ Kitchen Ready <span id="badge-count-ready" style="font-weight: 900; color: #10b981;">0</span>
                </button>
                <button class="filter-btn" data-filter="kitchen" onclick="setPackerFilter('kitchen')">
                    🔥 In Kitchen <span id="badge-count-kitchen" style="font-weight: 900; color: #f59e0b;">0</span>
                </button>
    ...
    <div class="kds-main-layout">
        ...
        <!-- RIGHT: LIVE PACKER OPERATIONS SIDEBAR -->
        <div class="kds-sidebar">
            ...
            <!-- SECTION B: 🎒 LIVE ARTICLES TO PACK ACCUMULATOR -->
            ...
            <div>
                <button onclick="fetchPackerOrders()" class="btn-action" style="width: 100%; padding: 10px; background: rgba(59, 130, 246, 0.15); border-color: rgba(59, 130, 246, 0.4); color: #93c5fd; font-weight: 800;">
                    🔄 Refresh Orders
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        let currentFilter = 'all'; // 'all', 'ready', 'kitchen', 'direct', 'urgent'
        ...
        function checkCompletionButton(orderId) {
            ...
            if (btn) {
                ...
                btn.style.color = canComplete ? '#0b0f19' : 'var(--text-muted)';
        ...
        function renderKdsWorkspace() {
            ...
            if (filteredOrders.length === 0) {
                workspace.innerHTML = `
                    <div class="empty-kds">
                        ...
                        <p style="margin: 0; font-size: 13px; color: var(--text-muted);">${activeOrdersList.length > 0 ? 'Try changing your filter settings above.' : 'All customer orders have been served.'}</p>
                    </div>
                `;
                return;
            }
            ...
            workspace.innerHTML = filteredOrders.map(order => {
                ...
                const customerName = order.customer_name || (order.client ? order.client.name : 'Counter Customer');

                const isKitchenReady = !order.has_kitchen_items || order.preparation_status === 'ready';

                // Kitchen status banners
        ...
        // --- 6. UPDATE SIDEBAR METRICS & ACCUMULATED ITEMS ---
        function updateSidebarAndBadges() {
            ...
            activeOrdersList.forEach(order => {
                ...
                // Collect items that still need to be placed in the bag
            ...
            // Update sidebar statistics boxes
            ...
            // Update 🎒 Live Items to Pack Accumulator
        ...
        // --- 7. TICKET TIMERS & URGENCY ALERTS ---
        ...
        // --- 8. REVERB WEBSOCKET LISTENER (CONFIG:CACHE SAFE) ---
